...
RSpec.describe Book, 'モデルに関するテスト', type: :model do
  ...
  context '空白のバリデーションチェック' do
    it 'titleが空白の場合にバリデーションチェックされ空白のエラーメッセージが返ってきているか' do
      book = described_class.new(title: '', body: 'hoge')
      expect(book).to be_invalid
      expect(book.errors[:title]).to include("can't be blank")
    end

    it 'bodyが空白の場合にバリデーションチェックされ空白のエラーメッセージが返ってきているか' do
      book = described_class.new(title: 'hoge', body: '')
      expect(book).to be_invalid
      expect(book.errors[:body]).to include("can't be blank")
    end
  end

  describe 'titleを空白で投稿した場合に画面にエラーメッセージが表示されているか' do
    before do
      visit books_path
  ...
  describe 'bodyを空白で投稿した場合に画面にエラーメッセージが表示されているか' do
    before do
      visit books_path
